 ...
 * Package:        com.dome.push.util
 * ClassName:      LanguageTool
 * Description:    描述
 * Author:         张继
 * CreateDate:     19-12-10 下午3:48
 * UpdateUser:     更新者
 * UpdateDate:     19-12-10 下午3:48
 * UpdateRemark:   更新说明
 * Version:        1.0
 */
public class LanguageTool {
    ...
    private static void applyLanguage(@NonNull final Locale locale,
                                      ...
                                      final boolean isFollowSystem) {
        if (isFollowSystem) {
            ApplicationTool.getSpUtils4Utils().put(KEY_LOCALE, VALUE_FOLLOW_SYSTEM);
        } else {
            String localLanguage = locale.getLanguage();
            String localCountry = locale.getCountry();
            ApplicationTool.getSpUtils4Utils().put(KEY_LOCALE, localLanguage + "$" + localCountry);
        }

        updateLanguage(ApplicationTool.getApp(), locale);

        Intent intent = new Intent();
        String realActivityClassName
                = TextUtils.isEmpty(activityClassName) ? getLauncherActivity() : activityClassName;
        intent.setComponent(new ComponentName(ApplicationTool.getApp(), realActivityClassName));
        intent.addFlags(
                Intent.FLAG_ACTIVITY_NEW_TASK
                        | Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_CLEAR_TASK
        );
        ApplicationTool.getApp().startActivity(intent);
    }

    static void applyLanguage(@NonNull final Activity activity) {
        final String spLocale = ApplicationTool.getSpUtils4Utils().getString(KEY_LOCALE);
        ...
        if (VALUE_FOLLOW_SYSTEM.equals(spLocale)) {
            ...
            updateLanguage(ApplicationTool.getApp(), sysLocale);
        ...
        updateLanguage(ApplicationTool.getApp(), settingLocale);
    ...
    private static String getLauncherActivity() {
        ...
        intent.setPackage(ApplicationTool.getApp().getPackageName());
